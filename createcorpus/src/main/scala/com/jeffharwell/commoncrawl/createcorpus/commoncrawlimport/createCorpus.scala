...
import org.apache.spark.sql.functions.{col, collect_list, desc, explode, size, udf}
...
object createCorpus extends java.io.Serializable {
  ...
  def main(args: Array[String]): Unit = {
    ...
    print("\n\n>>>>>> START OF PROGRAM <<<<<<\n\n")
    println("Creating Topical Corpus")
     ...
     */
    val target_tablespace = "pilotparse"  // tablespace in which to find the target tables
    val tables = List("abortion_presvm_20230512",
                      "guncontrol_presvm_20230512",
                      "evolution_presvm_20230512",
                      "existenceofgod_presvm_20230512")
    ...
    val number_of_raw_documents = 2000  // the number of raw documents to collect per query
    ...
    tables_and_queries.foreach(t_and_q => {
      ...
      val table_name = t_and_q._1
      val query_list = t_and_q._2
      val query = dh.formatQuery(query_list)
      log_messages += s"Processing Table: ${table_name}"
      ...
      //
      final_document_list.write.cassandraFormat(table_name, target_tablespace).mode("append").save()
