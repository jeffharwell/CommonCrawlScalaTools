...
object parseWETFilesFromCommoncrawl {
  def main(args: Array[String]) {
    ...
    print("\n\n>>>>>> START OF PROGRAM <<<<<<\n\n");
    println(s"Parsing $s_number_of_wet_paths Random WET Files from Common Crawl in batches of $batch_size");
    println("Running on Spark version: " + sc.version)

    val cc_url: String = "https://data.commoncrawl.org"

    // Creates a list containing the number of WET paths to pull for each batch of size $batch_size
    val batches: List[Int] = {
      if (number_of_wet_paths % batch_size == 0) {
        List.fill(number_of_wet_paths / batch_size)(batch_size)
      } else {
        List(number_of_wet_paths % batch_size) ++ List.fill(number_of_wet_paths / batch_size)(batch_size)
      }
    }

    println(s"Running ${batches.length} batches.")

    batches.foreach { batch_number_wet_paths =>

      // Get randow wetpaths that we have not processed yet
      val getwetpaths = new GetWETPaths(sc)
      val r = getwetpaths.getRandomWETPaths(batch_number_wet_paths)
    ...
    } // end of the list of batches

    println(s"Completed ${batches.length} batches.")
    print("\n\n>>>>>> END OF PROGRAM <<<<<<\n\n");
  }
}