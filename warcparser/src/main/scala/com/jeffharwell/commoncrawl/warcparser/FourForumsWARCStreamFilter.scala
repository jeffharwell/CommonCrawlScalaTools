...
class FourForumsWARCStreamFilter() extends WARCStreamFilter {
  ...
  def detailCheck[A <: WARCRecord](wrecord: A): Boolean = {
    ...
    */
    ...
    if (debug && valid_chunks.length > max_chunks_to_check) { println(s"Only searching to depth ${chunk_number_cutoff}") }

    @tailrec
    def checkChunks(chunks: ListBuffer[String], cutoff: Int): Boolean = {
      val chunk = chunks.head
      val t = chunks.tail

      // checkChunks is just looking for 1 mention of the keyword
      // in a more robust context. If the fast check already found the
      // minimum number of mentions then one mention in a sentence
      // is enough for us.
      if (containsKeywords(chunk, 1)) { true }
      else if (t.isEmpty || t.size < cutoff) { false } // we have fewer chunks left then our min, meaning we have checked
